...
std::string get_screen_id(std::string iconname, char delim = '|') {
  ...
  return (tokens.size() > 1)   ? tokens[1]
         : (tokens.size() > 0) ? (iconname.find("*") != std::string::npos) ? get_icon_name(tokens[0], '_') : tokens[0]
                               : "";
}
...
void EHMTX::bitmap_small(std::string icon, std::string text, int lifetime, int screen_time, bool default_font, int r,
                         int g, int b) {
  ...
  if (id == "") {
    ...
    ESP_LOGD(TAG, "small bitmap screen: id: %s text: %s lifetime: %d screen_time: %d", id.c_str(), text.c_str(),
             lifetime, screen_time);
  }
  screen->status();
}
...
void EHMTX::rainbow_bitmap_small(std::string icon, std::string text, int lifetime, int screen_time, bool default_font) {
  ...
  if (id == "") {
    ...
    ESP_LOGD(TAG, "small bitmap rainbow screen: text: %s lifetime: %d screen_time: %d", text.c_str(), lifetime,
             screen_time);
  } else {
    for (auto *t : on_add_screen_triggers_) {
      t->process("bitmap small: " + id, (uint8_t) screen->mode);
    }
    ESP_LOGD(TAG, "small bitmap rainbow screen: id: %s text: %s lifetime: %d screen_time: %d", id.c_str(), text.c_str(),
             lifetime, screen_time);
  }
  screen->status();
}
...
void EHMTX::bitmap_stack(std::string icons, int lifetime, int screen_time) {
  ...
  screen->text = ic;
  screen->progress =
      (id == "two") ? 1 : 0;  // 0 - one side scroll (right to left), 1 - two side (outside to center) if supported
...
void EHMTX::setup() {
  ...
  register_service(&EHMTX::set_infotext_color, "set_infotext_color",
                   {"left_r", "left_g", "left_b", "right_r", "right_g", "right_b", "default_font", "y_offset"});
  ...
  register_service(&EHMTX::icon_screen, "icon_screen",
                   {"icon_name", "text", "lifetime", "screen_time", "default_font", "r", "g", "b"});
  register_service(&EHMTX::alert_screen, "alert_screen",
                   {"icon_name", "text", "screen_time", "default_font", "r", "g", "b"});
  register_service(&EHMTX::rainbow_alert_screen, "rainbow_alert_screen",
                   {"icon_name", "text", "screen_time", "default_font"});
  register_service(&EHMTX::icon_clock, "icon_clock",
                   {"icon_name", "lifetime", "screen_time", "default_font", "r", "g", "b"});
  register_service(&EHMTX::icon_date, "icon_date",
                   {"icon_name", "lifetime", "screen_time", "default_font", "r", "g", "b"});
#ifdef USE_GRAPH
...
#endif
  register_service(&EHMTX::rainbow_icon_screen, "rainbow_icon_screen",
                   {"icon_name", "text", "lifetime", "screen_time", "default_font"});

  register_service(&EHMTX::icon_text_screen, "icon_text_screen",
                   {"icon_name", "text", "lifetime", "screen_time", "default_font", "r", "g", "b"});
  register_service(&EHMTX::rainbow_icon_text_screen, "rainbow_icon_text_screen",
                   {"icon_name", "text", "lifetime", "screen_time", "default_font"});

  register_service(&EHMTX::text_screen, "text_screen",
                   {"text", "lifetime", "screen_time", "default_font", "r", "g", "b"});
  register_service(&EHMTX::rainbow_text_screen, "rainbow_text_screen",
                   {"text", "lifetime", "screen_time", "default_font"});
  ...
  register_service(&EHMTX::bitmap_small, "bitmap_small",
                   {"icon", "text", "lifetime", "screen_time", "default_font", "r", "g", "b"});
  register_service(&EHMTX::rainbow_bitmap_small, "rainbow_bitmap_small",
                   {"icon", "text", "lifetime", "screen_time", "default_font"});
...
#ifdef EHMTXv2_ADV_CLOCK
  register_service(&EHMTX::set_clock_infotext_color, "set_clock_infotext_color",
                   {"left_r", "left_g", "left_b", "right_r", "right_g", "right_b", "default_font", "y_offset"});
  register_service(&EHMTX::set_date_infotext_color, "set_date_infotext_color",
                   {"left_r", "left_g", "left_b", "right_r", "right_g", "right_b", "default_font", "y_offset"});
  register_service(&EHMTX::set_adv_clock_color, "set_adv_clock_color",
                   {"hr", "hg", "hb", "mr", "mg", "mb", "sr", "sg", "sb"});
#endif

  register_service(&EHMTX::text_screen_progress, "text_screen_progress",
                   {"text", "value", "progress", "lifetime", "screen_time", "default_font", "value_color_as_progress",
                    "r", "g", "b"});
  register_service(&EHMTX::icon_screen_progress, "icon_screen_progress",
                   {"icon_name", "text", "progress", "lifetime", "screen_time", "default_font", "r", "g", "b"});
  register_service(&EHMTX::set_progressbar_color, "set_progressbar_color",
                   {"icon_name", "mode", "r", "g", "b", "bg_r", "bg_g", "bg_b"});

  register_service(&EHMTX::icon_prognosis_screen, "icon_prognosis_screen",
                   {"icon_name", "text", "prognosis", "lifetime", "screen_time", "default_font"});
  register_service(&EHMTX::icon_prognosis_screen_rgb, "icon_prognosis_screen_rgb",
                   {"icon_name", "text", "prognosis", "lifetime", "screen_time", "default_font", "r", "g", "b"});
...
void EHMTX::set_clock_color(int r, int g, int b) {
  ...
  for (size_t i = 0; i < MAXQUEUE; i++) {
    if (this->queue[i]->mode == MODE_CLOCK || this->queue[i]->mode == MODE_DATE ||
        this->queue[i]->mode == MODE_ICON_CLOCK || this->queue[i]->mode == MODE_ICON_DATE) {
...
void EHMTX::set_infotext_color(int lr, int lg, int lb, int rr, int rg, int rb, bool df, int y_offset) {
  ...
  ESP_LOGD(TAG, "info text color left: r: %d g: %d b: %d right: r: %d g: %d b: %d y_offset %d", lr, lg, lb, rr, rg, rb,
           y_offset);
#ifdef EHMTXv2_ADV_CLOCK
...
void EHMTX::force_screen(std::string icon_name, int mode) {
  for (uint8_t i = 0; i < MAXQUEUE; i++) {
    if (this->queue[i]->mode == mode) {
      bool force = true;
      if ((mode == MODE_ICON_SCREEN) || (mode == MODE_ICON_CLOCK) || (mode == MODE_ICON_DATE) ||
          (mode == MODE_FULL_SCREEN) || (mode == MODE_RAINBOW_ICON) || (mode == MODE_ICON_PROGRESS) ||
          (mode == MODE_ICON_TEXT_SCREEN) || (mode == MODE_RAINBOW_ICON_TEXT_SCREEN) || (mode == MODE_TEXT_PROGRESS) ||
#ifdef EHMTXv2_ADV_BITMAP
  ...
  if (this->clock->now().is_valid()) {
    ...
    for (size_t i = 0; i < MAXQUEUE; i++) {
      if ((this->queue[i]->endtime > 0.0) && (this->queue[i]->endtime < ts)) {
        ...
        if (this->queue[i]->mode != MODE_EMPTY) {
          ESP_LOGD(TAG, "remove expired queue element: slot %d: mode: %d icon_name: %s text: %s", i,
                   this->queue[i]->mode, this->queue[i]->icon_name.c_str(), this->queue[i]->text.c_str());
  ...
  if (this->is_running && this->clock->now().is_valid()) {
    ...
    if (ts > this->next_action_time) {
      ...
      if (this->screen_pointer != MAXQUEUE) {
        ...
        if (this->queue[this->screen_pointer]->mode == MODE_BITMAP_STACK_SCREEN &&
            this->queue[this->screen_pointer]->sbitmap != NULL) {
          for (uint8_t i = 0; i < this->queue[this->screen_pointer]->icon; i++) {
            this->icons[this->queue[this->screen_pointer]->sbitmap[i].b]->set_frame(0);
            this->queue[this->screen_pointer]->sbitmap[i] =
                Color(127, 255, this->queue[this->screen_pointer]->sbitmap[i].b, 5);
        ...
        this->next_action_time =
            this->queue[this->screen_pointer]->last_time + this->queue[this->screen_pointer]->screen_time_;
    ...
    if ((this->ticks_ <= EHMTXv2_BLEND_STEPS) && (this->brightness_ >= 50) && (this->queue_count() > 1)) {
      ...
      uint8_t current_step =
          ((b - 50) / 205) * (EHMTXv2_BLEND_STEPS - EHMTXv2_BLEND_STEPS / 2) + EHMTXv2_BLEND_STEPS / 2;
    ...
    defined EHMTXv2_ADV_BOOT_MODE_6 || defined EHMTXv2_ADV_BOOT_MODE_7
#if defined EHMTXv2_ADV_BOOT_MODE_6 || defined EHMTXv2_ADV_BOOT_MODE_7
...
void EHMTX::skip_screen() { this->next_action_time = this->get_tick() - 1000.0; }

void EHMTX::hold_screen(int time) { this->next_action_time = this->get_tick() + time * 1000.0; }

void EHMTX::get_status() {
  time_t ts = this->clock->now().timestamp;
  ESP_LOGI(TAG, "status time: %d.%d.%d %02d:%02d", this->clock->now().day_of_month, this->clock->now().month,
           this->clock->now().year, this->clock->now().hour, this->clock->now().minute);
...
void EHMTX::set_default_font(display::BaseFont *font) { this->default_font = font; }

void EHMTX::set_special_font(display::BaseFont *font) { this->special_font = font; }
...
void EHMTX::del_screen(std::string icon_name, int mode) {
  for (uint8_t i = 0; i < MAXQUEUE; i++) {
    if (this->queue[i]->mode == mode) {
      bool force = true;
      ESP_LOGD(TAG, "del_screen: icon %s in position: %s mode %d", icon_name.c_str(), this->queue[i]->icon_name.c_str(),
               mode);
      if ((mode == MODE_ICON_SCREEN) || (mode == MODE_ICON_CLOCK) || (mode == MODE_ICON_DATE) ||
          (mode == MODE_FULL_SCREEN) || (mode == MODE_RAINBOW_ICON) || (mode == MODE_ICON_PROGRESS) ||
          (mode == MODE_ICON_TEXT_SCREEN) || (mode == MODE_RAINBOW_ICON_TEXT_SCREEN) || (mode == MODE_TEXT_PROGRESS) ||
          (mode == MODE_BITMAP_SMALL) || (mode == MODE_RAINBOW_BITMAP_SMALL) ||
#ifdef EHMTXv2_ADV_BITMAP
          (mode == MODE_BITMAP_SCREEN) ||
#endif
          (mode == MODE_PROGNOSIS_SCREEN)) {
...
void EHMTX::alert_screen(std::string iconname, std::string text, int screen_time, bool default_font, int r, int g,
                         int b) {
  ...
  ESP_LOGD(TAG, "alert screen icon: %d iconname: %s text: %s screen_time: %d", icon, iconname.c_str(), text.c_str(),
           screen_time);
  screen->status();
...
void EHMTX::rainbow_alert_screen(std::string iconname, std::string text, int screen_time, bool default_font) {
  ...
  ESP_LOGD(TAG, "alert screen rainbow icon: %d iconname: %s text: %s screen_time: %d", icon, iconname.c_str(),
           text.c_str(), screen_time);
  screen->status();

  force_screen(iconname, MODE_RAINBOW_ALERT_SCREEN);
}

void EHMTX::icon_screen(std::string iconname, std::string text, int lifetime, int screen_time, bool default_font, int r,
                        int g, int b) {
  ...
  ESP_LOGD(TAG, "icon screen icon: %d iconname: %s text: %s lifetime: %d screen_time: %d", icon, iconname.c_str(),
           text.c_str(), lifetime, screen_time);
  screen->status();
}

void EHMTX::text_screen_progress(std::string text, std::string value, int progress, int lifetime, int screen_time,
                                 bool default_font, bool value_color_as_progress, int r, int g, int b) {
  ...
  ESP_LOGD(TAG, "text progress screen text: %s value: %s progress %d lifetime: %d screen_time: %d", text.c_str(),
           value.c_str(), progress, lifetime, screen_time);
  screen->status();
}

void EHMTX::icon_screen_progress(std::string iconname, std::string text, int progress, int lifetime, int screen_time,
  ...
  ESP_LOGD(TAG, "icon progress screen icon: %d iconname: %s text: %s progress %d lifetime: %d screen_time: %d", icon,
           iconname.c_str(), text.c_str(), progress, lifetime, screen_time);
  screen->status();
}
...
void EHMTX::set_progressbar_color(std::string iconname, int mode, int r, int g, int b, int bg_r, int bg_g, int bg_b) {
  ...
  screen->progressbar_color =
      (r + g + b == C_BLACK) ? esphome::display::COLOR_OFF : Color((uint8_t) r, (uint8_t) g, (uint8_t) b);
  screen->progressbar_back_color = (bg_r + bg_g + bg_b == C_BLACK)
                                       ? esphome::display::COLOR_OFF
                                       : Color((uint8_t) bg_r, (uint8_t) bg_g, (uint8_t) bg_b);

  ESP_LOGD(TAG,
           "progress screen mode: %d iconname: %s color progressbar: r: %d g: %d b: %d background: r: %d g: %d b: %d",
           mode, iconname.c_str(), r, g, b, bg_r, bg_g, bg_b);
  screen->status();
}
...
void EHMTX::icon_clock(std::string iconname, int lifetime, int screen_time, bool default_font, int r, int g, int b) {
  ...
  ESP_LOGD(TAG, "icon clock icon: %d iconname: %s lifetime: %d screen_time: %d", icon, iconname.c_str(), lifetime,
           screen_time);
  screen->status();
}
...
void EHMTX::icon_date(std::string iconname, int lifetime, int screen_time, bool default_font, int r, int g, int b) {
  ...
  ESP_LOGD(TAG, "icon date icon: %d iconname: %s lifetime: %d screen_time: %d", icon, iconname.c_str(), lifetime,
           screen_time);
  screen->status();
}

void EHMTX::rainbow_icon_screen(std::string iconname, std::string text, int lifetime, int screen_time,
                                bool default_font) {
  std::string ic = get_icon_name(iconname);
  std::string id = get_screen_id(iconname);
  ...
  ESP_LOGD(TAG, "rainbow icon screen icon: %d iconname: %s text: %s lifetime: %d screen_time: %d", icon,
           iconname.c_str(), text.c_str(), lifetime, screen_time);
  screen->status();
}
...
void EHMTX::full_screen(std::string iconname, int lifetime, int screen_time) {
  ...
  ESP_LOGD(TAG, "full screen: icon: %d iconname: %s lifetime: %d screen_time:%d ", icon, iconname.c_str(), lifetime,
           screen_time);
  screen->status();
}
...
void EHMTX::clock_screen(int lifetime, int screen_time, bool default_font, int r, int g, int b) {
  ...
  screen->text_color = Color(r, g, b);
  ESP_LOGD(TAG, "clock_screen_color lifetime: %d screen_time: %d red: %d green: %d blue: %d", lifetime, screen_time, r,
           g, b);
...
void EHMTX::icon_text_screen(std::string iconname, std::string text, int lifetime, int screen_time, bool default_font,
                             int r, int g, int b) {
  std::string ic = get_icon_name(iconname);
  std::string id = get_screen_id(iconname);
  ...
  ESP_LOGD(TAG, "icon text screen icon: %d iconname: %s text: %s lifetime: %d screen_time: %d", icon, iconname.c_str(),
           text.c_str(), lifetime, screen_time);
  screen->status();
}

void EHMTX::rainbow_icon_text_screen(std::string iconname, std::string text, int lifetime, int screen_time,
                                     bool default_font) {
  std::string ic = get_icon_name(iconname);
  std::string id = get_screen_id(iconname);
  ...
  ESP_LOGD(TAG, "rainbow icon text screen icon: %d iconname: %s text: %s lifetime: %d screen_time: %d", icon,
           iconname.c_str(), text.c_str(), lifetime, screen_time);
  screen->status();
}

void EHMTX::icon_prognosis_screen(std::string iconname, std::string text, std::string prognosis, int lifetime,
                                  int screen_time, bool default_font) {
  this->icon_prognosis_screen_rgb(iconname, text, prognosis, lifetime, screen_time, default_font, C_BLACK, C_BLACK,
                                  C_BLACK);
}

void EHMTX::icon_prognosis_screen_rgb(std::string iconname, std::string text, std::string prognosis, int lifetime,
                                      int screen_time, bool default_font, int r, int g, int b) {
  std::string ic = get_icon_name(iconname);
  std::string id = get_screen_id(iconname);
  ...
  ESP_LOGD(TAG, "icon prognosis screen icon: %d iconname: %s text: %s prognosis: %d lifetime: %d screen_time: %d", icon,
           iconname.c_str(), text.c_str(), static_cast<uint8_t>(i / 3), lifetime, screen_time);
  screen->status();
}

EHMTX_queue *EHMTX::find_icon_queue_element(uint8_t icon) {
  for (size_t i = 0; i < MAXQUEUE; i++) {
    if (((this->queue[i]->mode == MODE_ICON_SCREEN) || (this->queue[i]->mode == MODE_RAINBOW_ICON) ||
...
uint8_t EHMTX::get_brightness() { return this->brightness_; }

void EHMTX::set_clock_time(uint16_t t) { this->clock_time = t; }
...
void EHMTX::icon_graph_screen(std::string iconname, int lifetime, int screen_time) {
  ...
  ESP_LOGD(TAG, "graph screen with icon: icon: %d iconname: %s lifetime: %d screen_time:%d ", icon, iconname.c_str(),
           lifetime, screen_time);
  screen->status();
}
...
void EHMTX::draw_day_of_week(int ypos, bool small) {
  if (this->show_day_of_week) {
    ...
    if (this->weekday_accent && this->brightness_ < 50) {
      uint8_t max_diff = max3(this->today_color.r, this->today_color.g, this->today_color.b) -
                         max3(this->weekday_color.r, this->weekday_color.g, this->weekday_color.b);
    ...
    if (!small) {
      ...
      for (uint8_t i = 0; i <= 6; i++) {
        if (((!EHMTXv2_WEEK_START) && (dow == i)) ||
          ...
          if (accent_color != esphome::display::COLOR_OFF) {
            this->display->line((i < dow ? off_r : off_l) + i * 3, ypos + 7, (i < dow ? off_r : off_l) + i * 3,
                                ypos + 7, accent_color);
          }
        }
      }
...
void EHMTX::set_clock_infotext_color(int lr, int lg, int lb, int rr, int rg, int rb, bool df, int y_offset) {
  ...
  ESP_LOGD(TAG, "info clock text color left: r: %d g: %d b: %d right: r: %d g: %d b: %d y_offset %d", lr, lg, lb, rr,
           rg, rb, y_offset);
}
...
void EHMTX::set_date_infotext_color(int lr, int lg, int lb, int rr, int rg, int rb, bool df, int y_offset) {
  ...
  ESP_LOGD(TAG, "info date text color left: r: %d g: %d b: %d right: r: %d g: %d b: %d y_offset %d", lr, lg, lb, rr, rg,
           rb, y_offset);
}
...
void EHMTX::set_adv_clock_color(int hr, int hg, int hb, int mr, int mg, int mb, int sr, int sg, int sb) {
  ...
  ESP_LOGD(TAG, "advanced clock color hour: r: %d g: %d b: %d minutes: r: %d g: %d b: %d spacer: r: %d g: %d b: %d", hr,
           hg, hb, mr, mg, mb, sr, sg, sb);
}
  ...
  for (int i = 0; i < parts.size(); i++) {
    if (parts.at(i).length() > 0) {
      if (!(this->show_seconds && parts.at(i) == sep && (this->clock->now().second % 2 == 1))) {
        Color c_ = i == 0               ? this->hour_color
                   : i == 2             ? this->minutes_color
        ...
        if (i == 0)  // Hours
        {
          if (h != hours) {
            ...
            if (h % 10 != hours % 10) {
              this->display->printf(x + xstep, ypos + y - 8, font, c_, display::TextAlign::BASELINE_LEFT, "%d",
                                    (int) (h % 10));
              this->display->printf(x + xstep, ypos + y, font, c_, display::TextAlign::BASELINE_LEFT, "%d",
                                    (int) (hours % 10));
            } else {
        ...
        } else if (i == 2)  // Minutes
        {
          if (m != minutes) {
            ...
            if (m / 10 != minutes / 10) {
              ...
              this->display->printf(x, ypos + y, font, c_, display::TextAlign::BASELINE_LEFT, "%d",
                                    (int) (minutes / 10));
            } else {
    ...
    if (output.length() > 0) {
      if (output.find("%") != std::string::npos) {
        if (this->replace_time_date_active &&
            (output == "%a" || output == "%A" || output == "%b" || output == "%B"))  // check for replace active
        {
          output = this->clock->now().strftime(output);
          output = this->replace_time_date(output);
        } else {
...
void EHMTX::draw_text(std::string text, esphome::display::BaseFont *font, Color color, int xpos, int ypos) {
  ...
  for (int i = 0; i < res.size(); i++) {
    if (res.at(i).length() > 0) {
      int r, g, b;
      if (res.at(i).length() == 7 && std::regex_match(res.at(i), is_color) &&
          sscanf(&res.at(i).c_str()[1], "%02x%02x%02x", &r, &g, &b)) {
        if (r + g + b > 0) {
          c = Color(r, g, b);
        } else {
          c = color;
...
void EHMTX::draw_rainbow_text(std::string text, esphome::display::BaseFont *font, int xpos, int ypos) {
  ...
  for (uint16_t i = 0; i < str_len; i++) {
    ...
    this->display->printf(x + xpos, ypos, font, color, display::TextAlign::BASELINE_LEFT, "%s",
                          GetTextChar(text, i).c_str());
    x += this->GetTextWidth(font, "%s", GetTextChar(text, i).c_str());
  }
}
#endif

void EHMTX::set_weekday_char_count(uint8_t i) { this->weekday_char_count = i; }

std::string EHMTX::GetWeekdayChar(int position) { return GetTextChar(EHMTXv2_WEEKDAYTEXT, position); }
...
void EHMTX::add_icon(EHMTX_Icon *icon) {
  ...
  ESP_LOGD(TAG, "add_icon no.: %d name: %s frame_duration: %d ms", this->icon_count, icon->name.c_str(),
           icon->frame_duration);
  this->icon_count++;
}
        ...
        if ((this->queue[this->screen_pointer]->mode == MODE_ICON_CLOCK && this->icon_to_9 == 1) ||
          ...
          if (this->icon_indicator_height == 1) {
            this->display->line(4 - display_icon_indicator / 2, this->icon_indicator_y_pos,
                                4 + display_icon_indicator / 2, this->icon_indicator_y_pos, this->icon_indicator_color);
          } else {
          ...
          if (this->icon_indicator_height == 1) {
            this->display->line(4 - display_icon_indicator / 2, this->icon_indicator_y_pos,
                                3 + display_icon_indicator / 2, this->icon_indicator_y_pos, this->icon_indicator_color);
          } else {
            this->display->filled_rectangle(4 - display_icon_indicator / 2, this->icon_indicator_y_pos,
                                            this->display_icon_indicator, this->icon_indicator_height,
                                            this->icon_indicator_color);
...
void HOT EHMTX::draw() {
  if ((this->is_running) && (this->show_display)) {
    ...
    if (this->queue[this->screen_pointer]->mode != MODE_CLOCK && this->queue[this->screen_pointer]->mode != MODE_DATE &&
      ...
      if (this->queue[this->screen_pointer]->mode != MODE_ICON_SCREEN &&
          ...
          this->queue[this->screen_pointer]->mode != MODE_PROGNOSIS_SCREEN && !this->display_gauge) {
#endif
...
void EHMTXStartRunningTrigger::process() { this->trigger(); }

void EHMTXEmptyQueueTrigger::process() { this->trigger(); }

void EHMTXNextScreenTrigger::process(std::string iconname, std::string text) { this->trigger(iconname, text); }

void EHMTXAddScreenTrigger::process(std::string iconname, uint8_t mode) { this->trigger(iconname, mode); }

void EHMTXIconErrorTrigger::process(std::string iconname) { this->trigger(iconname); }

void EHMTXExpiredScreenTrigger::process(std::string iconname, std::string text) { this->trigger(iconname, text); }

void EHMTXNextClockTrigger::process() { this->trigger(); }

void EHMTXShowDisplayTrigger::process(bool state) { this->trigger(state); }

void EHMTXNightModeTrigger::process(bool state) { this->trigger(state); }
}  // namespace esphome
