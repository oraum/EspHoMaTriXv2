...
AUTO_LOAD = ["ehmtxv2_light", "json", "image", "animation"]
IMAGE_TYPE_RGB565 = 4
MAXFRAMES = 110
MAXICONS = 120
ICONWIDTH = 8
...
logging.warning("")
logging.warning(
    "Please check the documentation and wiki https://github.com/lubeda/EspHoMaTriXv2"
)
logging.warning("")
...
StartRunningTrigger = ehmtx_ns.class_(
    "EHMTXStartRunningTrigger", automation.Trigger.template()
)

EmptyQueueTrigger = ehmtx_ns.class_(
    "EHMTXEmptyQueueTrigger", automation.Trigger.template()
)

NextScreenTrigger = ehmtx_ns.class_(
    "EHMTXNextScreenTrigger", automation.Trigger.template()
)

IconErrorTrigger = ehmtx_ns.class_(
    "EHMTXIconErrorTrigger", automation.Trigger.template()
)

ExpiredScreenTrigger = ehmtx_ns.class_(
    "EHMTXExpiredScreenTrigger", automation.Trigger.template()
)

NextClockTrigger = ehmtx_ns.class_(
    "EHMTXNextClockTrigger", automation.Trigger.template()
)

AddScreenTrigger = ehmtx_ns.class_(
    "EHMTXAddScreenTrigger", automation.Trigger.template()
)

ShowDisplayTrigger = ehmtx_ns.class_(
    "EHMTXShowDisplayTrigger", automation.Trigger.template()
)

NightModeTrigger = ehmtx_ns.class_(
    "EHMTXNightModeTrigger", automation.Trigger.template()
)
...
EHMTX_SCHEMA = cv.Schema(
    {
        ...
        cv.Optional(CONF_CLOCKINTERVAL, default="0"): cv.templatable(cv.positive_int),
        cv.Optional(CONF_HTML, default=False): cv.boolean,
        cv.Optional(CONF_CACHE, default=False): cv.boolean,
        cv.Optional(CONF_RTL, default=False): cv.boolean,
        cv.Optional(CONF_VERTICAL, default=False): cv.boolean,
        cv.Optional(CONF_CLOCK, default=False): cv.boolean,
        cv.Optional(CONF_FLIP_FLOP, default=False): cv.boolean,
        cv.Optional(CONF_FLIP_FLOP_SPEED, default="2"): cv.templatable(
            cv.int_range(min=1, max=10)
        ),
        cv.Optional(CONF_BITMAP, default=False): cv.boolean,
        cv.Optional(CONF_BOOTLOGO): cv.string,
        cv.Optional(CONF_BOOTLOGOMODE, default="0"): cv.templatable(
            cv.int_range(min=0, max=7)
        ),
        cv.Optional(CONF_SHOW_SECONDS, default=False): cv.boolean,
        cv.Optional(CONF_WEEK_START_MONDAY, default=True): cv.boolean,
        cv.Optional(CONF_SHOWDOW, default=True): cv.boolean,
        cv.Optional(CONF_SCROLL_SMALL_TEXT, default=False): cv.boolean,
        cv.Optional(CONF_TIME_FORMAT, default="%H:%M"): cv.string,
        cv.Optional(CONF_TIME_FORMAT_BIG, default="%H:%M:%S"): cv.string,
        cv.Optional(CONF_WEEKDAYTEXT, default="SOMODIMIDOFRSA"): cv.string,
        cv.Optional(CONF_REPLACE_TIME_DATE_FROM, default=""): cv.string,
        cv.Optional(CONF_REPLACE_TIME_DATE_TO, default=""): cv.string,
        cv.Optional(CONF_ALWAYS_SHOW_RLINDICATORS, default=False): cv.boolean,
        cv.Optional(CONF_DATE_FORMAT, default="%d.%m."): cv.string,
        cv.Optional(CONF_DATE_FORMAT_BIG, default="%d.%m."): cv.string,
        cv.Optional(CONF_DEFAULT_FONT_XOFFSET, default="1"): cv.templatable(
            cv.int_range(min=-32, max=32)
        ),
        cv.Optional(CONF_DEFAULT_FONT_YOFFSET, default="6"): cv.templatable(
            cv.int_range(min=-32, max=32)
        ),
        cv.Optional(CONF_SPECIAL_FONT_XOFFSET, default="1"): cv.templatable(
            cv.int_range(min=-32, max=32)
        ),
        cv.Optional(CONF_SPECIAL_FONT_YOFFSET, default="6"): cv.templatable(
            cv.int_range(min=-32, max=32)
        ),
        cv.Optional(CONF_SCROLLINTERVAL, default="80"): cv.templatable(cv.positive_int),
        cv.Optional(CONF_BLENDSTEPS, default="0"): cv.templatable(cv.positive_int),
        cv.Optional(CONF_RAINBOWINTERVAL, default="32"): cv.templatable(
            cv.positive_int
        ),
        cv.Optional(CONF_RAINBOWSHIMMER, default=False): cv.boolean,
        cv.Optional(CONF_MULTICOLOR_TEXT, default=False): cv.boolean,
        cv.Optional(CONF_SCROLLCOUNT, default="2"): cv.templatable(cv.positive_int),
        cv.Optional(CONF_FRAMEINTERVAL, default="192"): cv.templatable(cv.positive_int),
        cv.Optional(CONF_BRIGHTNESS, default=80): cv.templatable(
        ...
        cv.Optional(
            CONF_NIGHT_MODE_SCREENS, default=DEFAULT_NIGHT_MODE_SCREENS
        ): cv.All(
            cv.ensure_list(
                cv.one_of(
                    1,
        ...
        cv.Optional(
            CONF_ICON_INDICATOR_SCREENS, default=DEFAULT_ICON_INDICATOR_SCREENS
        ): cv.All(
            cv.ensure_list(
                cv.one_of(
                    1,
            ...
            cv.ensure_list(
                {
                    ...
                    cv.Optional(CONF_FRAMEDURATION, default="0"): cv.templatable(
                        cv.positive_int
                    ),
...
async def to_code(config):
    ...
    logging.info("Preparing icons, this may take some seconds.")
...
"""
    ...
    for conf in config[CONF_ICONS]:
        if CONF_FILE in conf:
            ...
            path = CORE.relative_config_path(
                ".cache/icons/url/" + os.path.basename(a.path)
            )
            if config[CONF_CACHE] and os.path.isfile(path):
                try:
                    image = openImageFile(path)
        ...
        if ((width != 4 * ICONWIDTH) or (width != ICONWIDTH)) and (
            height != ICONHEIGHT
        ):
            logging.warning(
                f" icon wrong size valid 8x8 or 8x32: {conf[CONF_ID]} skipped!"
            )
        else:
            ...
            html_string += (
                f"<BR><B>{conf[CONF_ID]}</B>&nbsp;-&nbsp;({duration} ms):<BR>"
            )
    ...
    if config[CONF_HTML]:
        try:
            ...
            with open(htmlfn, "w") as f:
                ...
                f.close()
                logging.info(
                    f"EsphoMaTrix: wrote html-file with icon preview: {htmlfn}"
                )
    ...
    if config[CONF_WEEKDAYTEXT]:
        if (len(config[CONF_WEEKDAYTEXT])) == 7:
            ...
            logging.warning(
                "weekdays: must be 7 or 14 characters... your config may have unpredictable results!\n\r"
            )
    ...
    if config[CONF_REPLACE_TIME_DATE_FROM] and config[CONF_REPLACE_TIME_DATE_TO]:
        if (len(config[CONF_REPLACE_TIME_DATE_FROM].split())) != (
            len(config[CONF_REPLACE_TIME_DATE_TO].split())
        ):
            logging.warning(
                "replace_time_date_from: and replace_time_date_to: do not have matching pairs! (not using replacements)\n\r"
            )
            cg.add(var.set_replace_time_date_active(False))
        else:
            if (len(config[CONF_REPLACE_TIME_DATE_FROM].split())) > 50:
                logging.warning(
                    "replace_time_date_from: and replace_time_date_to: exceeds 30! (not using replacements)\n\r"
                )
                cg.add(var.set_replace_time_date_active(False))
            else:
                logging.info(
                    "replace_time_date_from: and replace_time_date_to: defined (using replacements)\n\r"
                )
        ...
        if config[CONF_REPLACE_TIME_DATE_TO]:
            logging.warning(
                "replace_time_date_to: defined but no replace_time_date_from:! (not using replacements)\n\r"
            )
        if config[CONF_REPLACE_TIME_DATE_FROM]:
            logging.warning(
                "replace_time_date_from: defined but no replace_time_date_to:! (not using replacements)\n\r"
            )
    ...
    for conf in config.get(CONF_ON_NEXT_SCREEN, []):
        ...
        await automation.build_automation(
            trigger, [(cg.std_string, "icon"), (cg.std_string, "text")], conf
        )
    ...
    for conf in config.get(CONF_ON_ADD_SCREEN, []):
        trigger = cg.new_Pvariable(conf[CONF_TRIGGER_ID], var)
        await automation.build_automation(
            trigger, [(cg.std_string, "icon"), (cg.uint8, "mode")], conf
        )
