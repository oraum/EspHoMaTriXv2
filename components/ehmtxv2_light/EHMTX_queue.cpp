...
void EHMTX_queue::status() {
  switch (this->mode) {
    ...
    case MODE_ICON_SCREEN:
      ESP_LOGD(TAG, "queue: icon screen: \"%s\" text: %s for: %.1f sec", this->icon_name.c_str(), this->text.c_str(),
               this->screen_time_ / 1000.0);
      break;
    case MODE_ICON_PROGRESS:
      ESP_LOGD(TAG, "queue: icon progress: \"%s\" text: %s for: %.1f sec", this->icon_name.c_str(), this->text.c_str(),
               this->screen_time_ / 1000.0);
      break;
    case MODE_PROGNOSIS_SCREEN:
      ESP_LOGD(TAG, "queue: icon prognosis screen: \"%s\" text: %s for: %.1f sec", this->icon_name.c_str(),
               this->text.c_str(), this->screen_time_ / 1000.0);
      break;
    case MODE_TEXT_PROGRESS:
      ESP_LOGD(TAG, "queue: text progress: \"%s\" text: %s for: %.1f sec", this->icon_name.c_str(), this->text.c_str(),
               this->screen_time_ / 1000.0);
      break;
    ...
    case MODE_RAINBOW_ALERT_SCREEN:
      ESP_LOGD(TAG, "queue: rainbow alert icon: \"%s\" for: %.1f sec", this->icon_name.c_str(),
               this->screen_time_ / 1000.0);
      break;
    ...
    case MODE_RAINBOW_ICON:
      ESP_LOGD(TAG, "queue: rainbow icon: \"%s\" text: %s for: %.1f sec", this->icon_name.c_str(), this->text.c_str(),
               this->screen_time_ / 1000.0);
      break;
    ...
    case MODE_ICON_TEXT_SCREEN:
      ESP_LOGD(TAG, "queue: icon text screen: \"%s\" text: %s for: %.1f sec", this->icon_name.c_str(),
               this->text.c_str(), this->screen_time_ / 1000.0);
      break;
    case MODE_RAINBOW_ICON_TEXT_SCREEN:
      ESP_LOGD(TAG, "queue: rainbow icon text screen: \"%s\" text: %s for: %.1f sec", this->icon_name.c_str(),
               this->text.c_str(), this->screen_time_ / 1000.0);
      break;
    case MODE_FIRE:
...
int16_t c8to16(uint8_t r, uint8_t g) { return (static_cast<uint16_t>(r) << 8) | g; }
...
int EHMTX_queue::xpos(uint8_t item) {
  ...
  if (this->icon < 5) {
    ...
    uint8_t target =
        round(((static_cast<float>(width) - 8 * static_cast<float>(this->icon)) / static_cast<float>(this->icon + 1)) *
                  (item + 1) +
              8 * item);
    if ((this->progress == 1) && (this->icon == 2 || this->icon == 3)) {
      uint8_t reverse_steps = round(
          ((static_cast<float>(width) - 8 * static_cast<float>(this->icon)) / static_cast<float>(this->icon + 1)) + 8);

      if (ceil((this->config_->next_action_time - this->config_->get_tick()) / EHMTXv2_SCROLL_INTERVAL) >
          reverse_steps) {
      ...
      uint8_t reverse_steps = round(((static_cast<float>(width) - 8 * static_cast<float>(this->icon)) /
                                     static_cast<float>(this->icon + 1)) *
                                        this->icon +
                                    8 * (this->icon + 1)) +
                              8;
  ...
  if (this->config_->is_running) {
    ...
    switch (this->mode) {
...
#ifdef EHMTXv2_USE_RTL
        this->config_->display->print(this->xpos() + xoffset, this->ypos() + yoffset, font, color_,
                                      esphome::display::TextAlign::BASELINE_RIGHT, this->text.c_str());
#else
        ...
        if (this->config_->clock->now().is_valid())  // valid time
        {
          ...
          if (!this->config_->draw_clock(EHMTXv2_TIME_FORMAT_BIG, font, color_, xoffset + 15, this->ypos() + yoffset)) {
            ...
            if (this->config_->replace_time_date_active)  // check for replace active
            {
              ...
              this->config_->display->printf(xoffset + 15, this->ypos() + yoffset, font, color_,
                                             display::TextAlign::BASELINE_CENTER, "%s", time_new.c_str());
            } else {
              this->config_->display->strftime(xoffset + 15, this->ypos() + yoffset, font, color_,
                                               display::TextAlign::BASELINE_CENTER, EHMTXv2_TIME_FORMAT,
          ...
          this->config_->display->print(15 + xoffset, yoffset, font, this->config_->alarm_color,
                                        display::TextAlign::BASELINE_CENTER, "!t!");
        }
        break;
        ...
        if (this->config_->clock->now().is_valid()) {
          ...
          if (!this->config_->draw_date(EHMTXv2_DATE_FORMAT_BIG, font, color_, xoffset + 15, this->ypos() + yoffset)) {
            ...
            if (this->config_->replace_time_date_active)  // check for replace active
            {
              ...
              this->config_->display->strftime(xoffset + 15, this->ypos() + yoffset, font, color_,
                                               display::TextAlign::BASELINE_CENTER, EHMTXv2_DATE_FORMAT,
                                               this->config_->clock->now());
            }
          ...
          this->config_->display->print(xoffset + 15, yoffset, font, this->config_->alarm_color,
                                        display::TextAlign::BASELINE_CENTER, "!d!");
        }
        break;
      ...
      case MODE_ICON_DATE:
        if (this->config_->clock->now().is_valid())  // valid time
        {
...
#ifdef EHMTXv2_ADV_CLOCK
            if (!this->config_->draw_clock(EHMTXv2_TIME_FORMAT, font, color_, xoffset + offset,
                                           this->ypos() + yoffset)) {
#endif
              if (this->config_->replace_time_date_active)  // check for replace active
              {
                std::string time_new = this->config_->clock->now().strftime(EHMTXv2_TIME_FORMAT).c_str();
                time_new = this->config_->replace_time_date(time_new);
                this->config_->display->printf(xoffset + offset, this->ypos() + yoffset, font, color_,
                                               display::TextAlign::BASELINE_CENTER, "%s", time_new.c_str());
              } else {
                this->config_->display->strftime(xoffset + offset, this->ypos() + yoffset, font, color_,
                                                 display::TextAlign::BASELINE_CENTER, EHMTXv2_TIME_FORMAT,
                                                 this->config_->clock->now());
              }
#ifdef EHMTXv2_ADV_CLOCK
...
#ifdef EHMTXv2_ADV_CLOCK
            if (!this->config_->draw_date(EHMTXv2_DATE_FORMAT, font, color_, xoffset + offset,
                                          this->ypos() + yoffset)) {
#endif
              if (this->config_->replace_time_date_active)  // check for replace active
              {
          ...
          if (this->icon != BLANKICON) {
            if (this->icon == SOLIDICON) {
              if ((this->mode == MODE_ICON_CLOCK && this->config_->icon_to_9 == 1) ||
                  (this->mode == MODE_ICON_DATE && this->config_->icon_to_9 == 2) || (this->config_->icon_to_9 == 3)) {
          ...
          if (this->icon_name.find("day") != std::string::npos ||
              this->icon_name.find("weekday") != std::string::npos) {
            ...
            if (this->icon_name.rfind("day", 0) == 0) {
              ...
              if (mode == 5 && (d < 10)) {
                ...
                this->config_->display->printf(x_right, this->ypos() + yoffset + i_y_offset, info_font, i_rcolor,
                                               display::TextAlign::BASELINE_LEFT, "%d", d % 10);
              } else {
                this->config_->display->printf(x_left, this->ypos() + yoffset + i_y_offset - (mode != 3 ? 0 : 1),
                                               info_font, i_lcolor, display::TextAlign::BASELINE_LEFT, "%d",
                                               d / 10 % 10);
                this->config_->display->printf(x_right, this->ypos() + yoffset + i_y_offset - (mode != 4 ? 0 : 1),
                                               info_font, i_rcolor, display::TextAlign::BASELINE_LEFT, "%d", d % 10);
              }
            } else  // if (this->icon_name.rfind("weekday", 0) == 0)
            {
              ...
              if (this->config_->weekday_char_count > 7) {
                ...
                this->config_->display->printf(x_left, this->ypos() + yoffset + i_y_offset - (mode != 3 ? 0 : 1),
                                               info_font, i_lcolor, display::TextAlign::BASELINE_LEFT, "%s",
                                               left.c_str());
                this->config_->display->printf(x_right, this->ypos() + yoffset + i_y_offset - (mode != 4 ? 0 : 1),
                                               info_font, i_rcolor, display::TextAlign::BASELINE_LEFT, "%s",
                                               right.c_str());
              } else {
                ...
                this->config_->display->printf(x_left, this->ypos() + yoffset + i_y_offset, info_font, i_lcolor,
                                               display::TextAlign::BASELINE_LEFT, "%s", weekday.c_str());
              }
            }
          }
        } else {
          this->config_->display->print(xoffset + 19, yoffset, font, this->config_->alarm_color,
                                        display::TextAlign::BASELINE_CENTER, "!t!");
        }
        break;
      ...
      case MODE_PROGNOSIS_SCREEN:
        ...
        color_ = (this->mode == MODE_RAINBOW_ICON || this->mode == MODE_RAINBOW_ALERT_SCREEN)
                     ? this->config_->rainbow_color
                     : this->text_color;
#ifdef EHMTXv2_USE_RTL
        this->config_->display->print(this->xpos() + xoffset, this->ypos() + yoffset, font, color_,
                                      esphome::display::TextAlign::BASELINE_RIGHT, this->text.c_str());
        ...
        if (this->mode == MODE_ICON_PROGRESS) {
          ...
          if (this->progress != 0) {
            if (this->progressbar_color == esphome::display::COLOR_OFF) {
              color_ = esphome::light::ESPHSVColor(this->progress * 96 / 100 + (this->progress < 0 ? 96 : 0), 255, 240)
                           .to_rgb();
            } else {
              color_ = this->progressbar_color;
            }
            this->config_->display->line(9, this->ypos() + 7, 9 + abs(this->progress) * 22 / 100, this->ypos() + 7,
                                         color_);
          }
      ...
      case MODE_TEXT_PROGRESS:
        ...
        this->config_->display->print(0, this->ypos() + yoffset, font, color_,
                                      esphome::display::TextAlign::BASELINE_LEFT, this->icon_name.c_str());

        this->config_->display->line(0, this->ypos() + 7, 31, this->ypos() + 7, this->progressbar_back_color);
        if (this->progress != 0) {
          if (this->progressbar_color == esphome::display::COLOR_OFF) {
            color_ = esphome::light::ESPHSVColor(this->progress * 120 / 100 + (this->progress < 0 ? 120 : 0), 255, 240)
                         .to_rgb();
          } else {
            color_ = this->progressbar_color;
          }
        ...
        this->config_->display->print(32, this->ypos() + yoffset, font, color_,
                                      esphome::display::TextAlign::BASELINE_RIGHT, this->text.c_str());
        break;
        ...
        if (this->sbitmap != NULL) {
          for (uint8_t i = 0; i < this->icon; i++) {
            if (this->sbitmap[i].b != BLANKICON) {
              if (this->icon == SOLIDICON) {
                this->config_->display->filled_rectangle(this->xpos(i), this->ypos(i), 8, 8,
                                                         this->config_->solid_color);
              } else if (this->icon == CALENDARICON) {
                this->config_->display->filled_rectangle(this->xpos(i), this->ypos(), 8, 8,
                                                         Color(C_RED, C_GREEN, C_BLUE));
                this->config_->display->filled_rectangle(this->xpos(i), this->ypos(), 8, 2,
                                                         this->config_->calendar_color);
              } else {
...
void EHMTX_queue::calc_scroll_time(std::string text, uint16_t screen_time) {
  ...
  if (this->default_font) {
    this->config_->display->get_text_bounds(0, 0, text_.c_str(), this->config_->default_font, display::TextAlign::LEFT,
                                            &x, &y, &w, &h);
  } else {
    this->config_->display->get_text_bounds(0, 0, text_.c_str(), this->config_->special_font, display::TextAlign::LEFT,
                                            &x, &y, &w, &h);
  }
  ...
  ESP_LOGD(TAG, "calc_scroll_time: mode: %d text: \"%s\" pixels %d calculated: %.1f defined: %d max_steps: %d",
           this->mode, text.c_str(), this->pixels_, this->screen_time_ / 1000.0, screen_time, this->scroll_reset);
}
...
void EHMTX_queue::calc_scroll_time(uint8_t icon_count, uint16_t screen_time) {
  ...
  ESP_LOGD(TAG, "calc_scroll_time: mode: %d icons count: %d pixels %d calculated: %.1f defined: %d max_steps: %d",
           this->mode, icon_count, this->pixels_, this->screen_time_ / 1000.0, screen_time, this->scroll_reset);
}
}  // namespace ehmtx
}  // namespace esphome
